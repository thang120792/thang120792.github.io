...
function exportToWord() {
    ...
    const rawText = formOutputEl ? formOutputEl.value : '';
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (trimmed.startsWith('|') && trimmed.endsWith('|')) {
            ...
            tableRows.forEach((row, rIdx) => {
                ...
                row.forEach(cell => {
                    bodyHtml += `<td style="border:1px solid #000; padding:4px 6px;">${cell || '&nbsp;'}</td>`;
                });
                bodyHtml += `</tr>`;
            });
            bodyHtml += `</table>`;
        ...
        if (trimmed.includes('CỘNG HÒA XÃ HỘI CHỦ NGHĨA VIỆT NAM') || trimmed.includes('CỘNG HOÀ XÃ HỘI CHỦ NGHĨA VIỆT NAM')) {
            ...
            bodyHtml += `<div style="font-family:'Times New Roman', serif; font-size:11pt; font-weight:bold; margin-bottom:4px;">${trimmed}</div>`;
        } else if (trimmed.startsWith('(Ban hành kèm theo') || trimmed.startsWith('(Kèm theo')) {
            bodyHtml += `<div style="font-family:'Times New Roman', serif; font-size:10pt; font-style:italic; margin-bottom:8px;">${trimmed}</div>`;
            ...
            <table width="100%" border="0" cellspacing="0" cellpadding="0" style="border-collapse:collapse; margin-top:20px; font-family:'Times New Roman', serif;">
                <tr>
                    ...
                    <td width="50%" align="center" valign="top">
                        <div style="font-weight:bold; font-size:11.5pt; text-transform:uppercase;">${trimmed}</div>
                        <div style="font-size:10pt; font-style:italic; margin-top:2px;">(Ký, ghi rõ họ tên và đóng dấu nếu có)</div>
                        <div style="height:60px;"></div>
                        <div style="font-weight:bold; font-size:12pt;">${cccdHoten && !cccdHoten.includes('.') ? cccdHoten : ''}</div>
                    </td>
                </tr>
            </table>`;
        } else {
    ...
    try {
        ...
        console.log("✅ Đã xuất chuẩn xác 100% nội dung mẫu đơn sang Word:", filename);
    } catch (err) {
