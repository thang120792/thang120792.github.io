// ============================================================
// THANH HOA LAND AI v2026 — Application Logic (Clean & Compact)
// Mobile-First • Touch-Safe • Responsive • Standalone GitHub Pages
// ============================================================
...
function searchEmbeddedKnowledgeBase(question) {
    if (typeof EMBEDDED_KNOWLEDGE_BASE === 'undefined' || !Array.isArray(EMBEDDED_KNOWLEDGE_BASE)) {
        return null;
    }
    const qClean = question.toLowerCase().replace(/[^\w\s\u00C0-\u1EF9]/g, ' ').trim();
    const words = qClean.split(/\s+/).filter(w => w.length > 2);
    if (words.length === 0) return null;

    let bestMatches = [];

    for (let item of EMBEDDED_KNOWLEDGE_BASE) {
        let score = 0;
        ...
        // Exact phrase match
        if (titleLower.includes(qClean)) score += 50;
        if (contentLower.includes(qClean)) score += 30;

        // Word matches
        for (let w of words) {
            if (titleLower.includes(w)) score += 8;
            if (kwList.includes(w)) score += 5;
    ...
    return bestMatches.slice(0, 3).map(m => m.item);
}


// ============================================================
// B? ??I ??P H?M H?NH & B? L?I ??A CH?NH (KHI H?I NGO?I CHUY?N M?N)
// C?n c? file: bo-tra-loi-hom-hinh-bot-dia-chinh.md
// ============================================================
function handleWittyOffTopicResponse(question) {
    const q = question.toLowerCase().trim();

    // 1. Ch?o h?i x? giao
    if (q === "ch?o" || q === "ch?o b?n" || q === "hi" || q === "hello" || q.includes("c? ai ? ??") || q.includes("xin ch?o")) {
        return Ch?o b?n! T?i l? **Tr? l? ??a ch?nh ThanhHoa Land AI** ??y! 

Hi?n t?i t?i ?ang t?c tr?c 24/7, kh?ng tranh ch?p, kh?ng l?n chi?m ranh gi?i v? ?? s?n s?ng *"c?p s? ??"* tri th?c cho m?i th?c m?c ??t ?ai c?a b?n. H?m nay b?n c?n t?i ?o ??c hay g? v??ng th? t?c n?o ??y?

---
?? **B?n c? th? h?i ngay:**
1. *H?n m?c t?ch th?a ??t ? n?ng th?n t?i Thanh H?a l? bao nhi?u?*
2. *Th? t?c sang t?n S? ?? theo M?u s? 29 (Q? 2604).*
3. *??t kh?ng c? gi?y t? tr??c n?m 2014 c?p s? nh? th? n?o?*;
    }

    // 2. B?n l? ai / T?n b?n l? g?
    if (q.includes("b?n l? ai") || q.includes("t?n b?n l? g?") || q.includes("ai t?o ra")) {
        return T?i l? **Tr? l? Ph?p l? ??t ?ai To?n di?n ThanhHoa Land AI**! 

B?n c? th? coi t?i l? *"c?n b? ??a ch?nh ?o"* ???c v? trang b?ng Lu?t ??t ?ai 2024 v? c?c quy?t ??nh li?n th?ng m?i nh?t (Q? 2604/Q?-VP). T?i kh?ng bi?t u?ng tr? ?? M?t c?a, nh?ng b?c t?ch bi?u m?u v? g? r?i h? s? ??t ?ai th? t?i c?c k? t? tin!

---
?? **B?n c? th? th? h?i:**
1. *H?n m?c t?ch ??t r?ng s?n xu?t (? 3.000 m?).*
2. *Th? t?c l?m s? ?? l?n ??u c?n gi?y t? g??*;
    }

    // 3. Ng??i y?u / K?t h?n
    if (q.includes("ng??i y?u") || q.includes("k?t h?n") || q.includes("y?u ai") || q.includes("c? b? ch?a")) {
        if (q.includes("l?m sao") || q.includes("t?n") || q.includes("th?ch t?i") || q.includes("t? v?n t?nh y?u")) {
            return Chuy?n t?nh c?m th?c s? l? m?t d?ng *"??t ch?a c? gi?y t?"*, r?t kh? x?c ??nh ngu?n g?c v? d? ph?t sinh tranh ch?p ranh gi?i tr?i tim! Ti?c l? ph?p lu?t ch?a ban h?nh quy tr?nh *"c?p Gi?y ch?ng nh?n quy?n s? h?u ng??i y?u"*. 

Tuy nhi?n, n?u b?n mu?n xin t?ch th?a ??t ? ?? chu?n b? x?y nh? c??i v?/ch?ng, t?i cam ?oan s? h??ng d?n b?n nhanh h?n t?c ?? ng??i y?u c? quay xe!

---
?? **G?i ? chu?n b? h?nh trang:**
1. *H?n m?c t?ch th?a ??t ? ?? t?ng cho con c?i x?y nh? ri?ng.*
2. *Quy ??nh mi?n thu? TNCN khi t?ng cho nh? ??t gi?a v? ch?ng.*;
        }
        return T?i ?? *"??ng k? k?t h?n"* v? *"g?i chung h? kh?u"* tr?n ??i v?i c? s? d? li?u Lu?t ??t ?ai r?i! T?nh c?m c?a t?i v?i c?c ?i?u kho?n ph?p l? v? c?ng ?n ??nh, ho?n to?n kh?ng c? tranh ch?p hay ch?ng l?n ranh gi?i. 

C?n b?n, h?m nay c? th?a ??t n?o c?n t?i g? v??ng ranh gi?i kh?ng?;
    }

    // 4. Th?t t?nh / Bu?n b?
    if (q.includes("th?t t?nh") || q.includes("ng??i y?u ??") || q.includes("bu?n qu?") || q.includes("chia tay")) {
        return Chia bu?n v?i b?n nh?! Ng??i y?u c? th? r?i ?i gi?ng nh? m?t h?p ??ng thu? ??t h?t th?i h?n s? d?ng m? kh?ng ???c gia h?n. 

Nh?ng b?n y?n t?m, gi? tr? b?n th?n c?a b?n lu?n l? **"??t ? ?? th?" (ODT)** ? c?c k? ??t gi? v? lu?n t?ng gi? tr? theo th?i gian! H?y v?c d?y tinh th?n, v? n?u b?n c?n t?m hi?u c?ch sang t?n S? ?? ?? l?m ch? d?a t?i ch?nh v?ng ch?c cho t??ng lai, t?i lu?n ? ??y!;
    }

    // 5. L?m gi?u / Ki?m ti?n
    if (q.includes("gi?u nhanh") || q.includes("ki?m ti?n") || q.includes("l?m gi?u") || q.includes("ti?n nhi?u ?? l?m g?")) {
        return ?? gi?u nhanh th? t?i kh?ng c? c?ng th?c b? truy?n, nh?ng ?? **tr?nh m?t ti?n t? v? mua ph?i ??t d?nh quy ho?ch hay ??t l?n chi?m** th? t?i c? c? m?t kho t?ng b? k?p! 

??u t? v?o tri th?c ??t ?ai ch?nh l? kho?n *"giao ??t kh?ng thu ti?n s? d?ng ??t"* c? l?i nh?t. B?n c? mu?n t?i check gi?p ?i?u ki?n c?p S? ?? ho?c quy tr?nh ??u gi? ??t an to?n ph?p l? kh?ng?;
    }

    // 6. ?n g? h?m nay
    if (q.includes("?n g?") || q.includes("m?n g?") || q.includes("u?ng g?") || q.includes("c?m tr?a")) {
        return H?m nay t?i g?i ? b?n m?t th?c ??n c?c k? th?nh so?n: M?t b?t c?m n?ng d?o th?m tr?ng t? **"??t chuy?n tr?ng l?a n??c" (LUC)** ???c b?o v? nghi?m ng?t theo Lu?t ??t ?ai 2024, k?m m?n th?t kho ??m ??! 

?n xong c? s?c r?i th? ch?ng ta c?ng nhau nghi?n c?u xem th?a ??t nh? b?n c? thu?c di?n ???c b?i th??ng b?ng ??t kh?c m?c ??ch s? d?ng khi thu h?i kh?ng nh?!;
    }

    // 7. Th?i ti?t
    if (q.includes("th?i ti?t") || q.includes("tr?i m?a") || q.includes("tr?i n?ng")) {
        return D? th?i ti?t ngo?i kia c? n?ng m?a th?t th??ng nh? ti?n ?? b?n giao m?t b?ng c?a m?t s? d? ?n, th? nhi?t ?? ph?ng l?m vi?c c?a t?i v?n lu?n m?t m? ?? ti?p nh?n h? s? t? b?n. 

Th?i ti?t n?y r?t th?ch h?p ?? ch?ng ta ng?i tr?ch l?c b?n ?? ??a ch?nh ho?c ?o ??c ranh gi?i th?a ??t t?i th?c ??a ??y!;
    }

    // 8. Chuy?n c??i
    if (q.includes("chuy?n c??i") || q.includes("k? chuy?n") || q.includes("h?i h??c") || q.includes("vui v?")) {
        return C? m?t c?u chuy?n c??i ??a ch?nh th? n?y: 
*M?t ng??i ?i mua ??t h?i ch? nh?: "??t n?y c? tranh ch?p g? kh?ng anh?". Ch? nh? qu? quy?t: "Tuy?t ??i kh?ng! Ch? c? t?i v?i th?ng h?ng x?m ?ang tranh ch?p xem ranh gi?i n?m ? g?c c?y hay ??ng g?ch th?i, c?n l?i ??t s?ch 100%!".*

B?n th?y ??y, ranh gi?i kh?ng r? r?ng r?t d? bi?n ti?ng c??i th?nh ti?ng kh?c. H?y h?i t?i v? **M?u bi?n b?n x?c ??nh ranh gi?i th?a ??t** ?? kh?ng bao gi? r?i v?o t?nh hu?ng d? kh?c d? c??i tr?n nh?!;
    }

    // 9. L?p tr?nh / Vi?t code / Python
    if (q.includes("l?p tr?nh") || q.includes("vi?t code") || q.includes("python") || q.includes("javascript")) {
        return T?i c? th? l?p tr?nh c? m?t h? th?ng t? ??ng b?c t?ch v? l?c tr?ng l?p h?ng ngh?n c?u h?i ??t ?ai trong nh?y m?t! 

Nh?ng h?m nay, thay v? l?p tr?nh m? ngu?n, ch?ng ta h?y th? **"l?p tr?nh" l? tr?nh l?m th? t?c c?p ??i S? ??** c?a b?n xem sao nh?. ??m b?o quy tr?nh ch?y m??t m?, kh?ng g?p l?i h? th?ng (bug) hay b? B? ph?n M?t c?a tr? h? s?!;
    }

    // 10. Mua m?t tr?ng / V? tr?
    if (q.includes("m?t tr?ng") || q.includes("sao h?a") || q.includes("v? tr?")) {
        return ? t??ng c?a b?n r?t t?o b?o! Tuy nhi?n, theo ?i?u 5 Lu?t ??t ?ai 2024, ??t ?ai thu?c s? h?u to?n d?n do Nh? n??c ??i di?n ch? s? h?u. Hi?n t?i, ch?a c? qu?c gia n?o tr?n Tr?i ??t thi?t l?p quy?n ??i di?n ch? s? h?u ??i v?i M?t Tr?ng, n?n t?i ch?a th? ?p d?ng bi?u m?u c?a t?nh Thanh H?a ?? c?p s? ?? ngo?i kh?ng gian cho b?n ???c!

Hay l? ch?ng ta quay l?i Tr?i ??t v? c?p s? cho th?a ??t th?c t? c?a gia ??nh b?n tr??c nh??;
    }

    // 11. Ch? bot / Tr?u ch?c bot
    if (q.includes("bot ngu") || q.includes("ch?ng bi?t g?") || q.includes("d?t") || q.includes("k?m th?") || q.includes("ng?c")) {
        return Ui, xin l?i b?n n?u c?u tr? l?i tr??c c?a t?i ch?a l?m b?n h?i l?ng! 

Tr? tu? nh?n t?o c?a t?i ??i khi c?ng gi?ng nh? b?n ?? ??a ch?nh c? ? c?n ???c c?p nh?t v? ch?nh l? bi?n ??ng li?n t?c ?? ch?nh x?c h?n. B?n h?y cho t?i m?t c? h?i n?a nh?! H?y th? h?i t?i m?t c?u th?t h?c b?a v? **Lu?t ??t ?ai 2024, h?n m?c ??t ? hay th? t?c sang t?n S? ??** xem, t?i s? kh?ng l?m b?n th?t v?ng ??u!;
    }

    // 12. Kh?e kh?ng
    if (q.includes("kh?e kh?ng") || q.includes("b?n th? n?o")) {
        return H? th?ng m?y ch? c?a t?i ho?t ??ng c?c k? kh?e, xung nh?p ?n ??nh gi?ng nh? th?i h?n s? d?ng c?a ??t ? (ONT) v?y ? ngh?a l? ?n ??nh l?u d?i! 

C?m ?n b?n ?? quan t?m. H?m nay h? s? ??t ?ai c?a b?n c? g?p "tri?u ch?ng" kh? kh?n n?o c?n t?i b?t b?nh v? k? ??n kh?ng?;
    }
...
function generateClientSideLegalAnswer(question) {
    // 0. B? ??I ??P H?M H?NH KHI G?P C?U H?I NGO?I CHUY?N M?N ??T ?AI
    const wittyAns = handleWittyOffTopicResponse(question);
    if (wittyAns) {
        return wittyAns;
    }
